...
面向中文用户的非官方 VRChat 本机伴侣。项目采用 **Vue Web UI + Go 本机网关**：页面负责交互，本机网关负责 VRChat 会话、API、Pipeline、游戏日志和本地数据，避免把用户凭据与会话集中上传到第三方服务器。

> 当前版本：`0.9.0-beta.1`。这是测试版本，不是 VRChat 官方产品，也不能替代 VRChat 客户端。VRChat API 与平台规则可能变化，请在使用和发布前重新核对上游要求。
...
- Windows 托盘、版本检查、双源下载、哈希校验与自更新主体；
...
# 构建前必须设置可联系的 User-Agent；占位邮箱会被拒绝用于真实登录
...
程序会启动本机网关，并默认打开内嵌的 WebView2 窗口；缺少 WebView2 Runtime 时会回退到默认浏览器。开发模式、启动参数、数据目录和代理配置见 [本地开发说明](docs/development.md)。
...
- 安装包常驻、Authenticode 正式签名；
- 线上双源更新和不同地区/运营商网络表现。

详细进度以 [实现状态](docs/implementation-status.md) 为准；版本变化见 [0.9.0-beta.1 发布说明](docs/release-notes-0.9.0-beta.1.md)。
