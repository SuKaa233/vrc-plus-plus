...
import { Activity, AlertTriangle, Eye, Globe2, History, LoaderCircle, MapPin, Network, RefreshCw, ScanSearch, ShieldCheck, UserRound, Users } from '@lucide/vue'
...
<template>
  <article class="concerned-view">
    <header class="focus-header panel">
      ...
      <label>关注好友<select v-model="selectedID"><option v-for="item in friends" :key="item.id" :value="item.id">{{ item.online ? '● ' : '' }}{{ item.displayName }}</option></select></label>
