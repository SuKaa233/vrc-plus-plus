...
import { CircleAlert, Copy, ExternalLink, Globe2, Heart, LoaderCircle, Play, Send, Server, Star, Users, X } from '@lucide/vue'
...
const currentInstanceFriends = computed(() => props.instanceLocation ? props.friends.filter((friend) => friend.location === props.instanceLocation) : [])
const sameWorldFriends = computed(() => props.friends.filter((friend) => friend.location?.startsWith(`${props.world.id}:`) && friend.location !== props.instanceLocation))
const launchState = computed(() => {
  if (!props.instanceLocation) return { state: 'blocked', label: '尚未选择实例' }
  if (props.instance && !props.instance.active) return { state: 'blocked', label: '实例已失效或不可用' }
  if (props.instance?.full) return { state: 'warn', label: props.instance.queueEnabled ? '实例已满，可尝试进入队列' : '实例已满，启动后可能失败' }
  if (props.instanceError) return { state: 'warn', label: '详情受权限或网络限制，但仍可使用已有位置启动' }
  return { state: 'ready', label: '实例可启动；最终权限由 VRChat 客户端确认' }
})
const canLaunch = computed(() => !!props.instanceLocation && props.instance?.active !== false)
...
function launch() { if (!canLaunch.value || !window.confirm(`即将由 VRChat 启动并加入“${props.world.name}”。继续吗？`)) return; window.location.href = `vrchat://launch?id=${encodeURIComponent(props.instanceLocation)}` }
</script>
    ...
    <aside class="world-detail" role="dialog" aria-modal="true" :aria-label="`${world.name} 世界详情`">
      ...
      <div class="world-detail-body">
        ...
        <section class="instances-panel">
          ...
          <div v-else-if="instanceError" class="instance-error"><CircleAlert :size="16" />{{ instanceError }}</div>
          ...
          <div v-if="instanceLocation" class="join-readiness" :data-state="launchState.state"><CircleAlert :size="14"/>{{ launchState.label }}</div>
          <div v-if="currentInstanceFriends.length || sameWorldFriends.length" class="instance-friends"><strong><Users :size="14"/>好友线索</strong><span v-if="currentInstanceFriends.length">同一实例：{{ currentInstanceFriends.map(friend=>friend.displayName).slice(0,5).join('、') }}</span><span v-if="sameWorldFriends.length">同世界其他实例：{{ sameWorldFriends.map(friend=>friend.displayName).slice(0,5).join('、') }}</span></div>
          <div v-if="instanceLocation" class="instance-actions"><button @click="copyLocation"><Copy :size="15" />{{ copied || '复制位置' }}</button><button class="join" :disabled="!canLaunch" @click="launch"><Play :size="15" />启动并加入</button></div>
        </section>
