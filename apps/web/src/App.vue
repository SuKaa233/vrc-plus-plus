...
const statusCopy = computed(() => {
  if (session.value.status === "authenticated")
    return l("已连接 VRChat", "VRChat connected");
  if (session.value.status === "two_factor_required")
    return l("等待二步验证", "Two-factor authentication required");
  if (session.value.status === "anonymous")
    return l("尚未登录", "Not signed in");
...
const realtimeLabel = computed(() => {
  if (realtime.value.state === "connected")
    return l("实时已连接", "Realtime connected");
  if (realtime.value.state === "connecting")
    return l("实时连接中", "Connecting realtime");
  if (realtime.value.state === "disconnected")
    return l("实时重连中", "Reconnecting realtime");
  return l("实时未启用", "Realtime disabled");
});

const onlineFriends = computed(
  () => friends.value?.items.filter((friend) => friend.online).length ?? 0,
);
...
const visibleWorlds = computed(() => {
  ...
  for (const item of Object.values(hydratedWorlds.value))
    byID.set(item.id, item);
  for (const item of Object.values(worldFavorites.value))
    byID.set(item.world.id, item.world);
  for (const item of Object.values(upstreamWorldFavorites.value))
    if (!byID.has(item.world.id)) byID.set(item.world.id, item.world);
  for (const item of worlds.value?.items ?? [])
...
const selectedAnnotation = computed(() =>
  selectedFriend.value
    ? (annotations.value[selectedFriend.value.id] ?? null)
    : null,
);
const selectedWorldFavorite = computed(() =>
  selectedWorld.value
    ? (worldFavorites.value[selectedWorld.value.id] ?? null)
    : null,
);
const selectedUpstreamFavorite = computed(() =>
  selectedWorld.value
    ? (upstreamWorldFavorites.value[selectedWorld.value.id] ?? null)
    : null,
);
  ...
  () =>
    (locale.value === "en"
      ? {
          overview: [
            "Dashboard",
            `Hello, ${session.value.user?.displayName ?? "Traveler"}`,
            "A clear view of today.",
          ],
          compass: [
            "Compass",
            "Where to tonight",
            "Worlds that fit your current friends.",
          ],
          journey: [
            "Journey",
            "Friend motion and trails",
            "See recent movement and circle intersections.",
          ],
          friends: [
            "Friends",
            "Friend activity",
            "Find, filter and review friends.",
          ],
          focus: [
            "Watchlist",
            "Concerned friend dossier",
          ...
          network: [
            "Network",
            "Friend network",
            "Explore circles and mutual links.",
          ],
          worlds: [
            "Worlds",
            "Worlds and instances",
            "Discover worlds you can join.",
          ],
          groups: ["Groups", "Group center", "Groups, events and posts."],
          avatars: [
            "Avatars",
            "Avatar collection",
            "Organize favorite avatars.",
          ],
          activity: ["History", "Activity history", "What happened recently."],
          notifications: [
            "Notifications",
            "Requests and invites",
            "Handle notifications and friend requests.",
          ],
        }
      : {
          overview: [
            "工作台",
            `你好，${session.value.user?.displayName ?? "旅行者"}`,
            "今天有什么新情况。",
          ],
...
function l(chinese: string, english: string) {
  return interfaceText(locale.value, chinese, english);
}
...
async function saveNetwork() {
  ...
  error.value = "";
  try {
    const value = await api.updateNetwork(
      networkMode.value,
      proxyUrl.value.trim(),
    );
...
function recentKey() {
  return `vrc-harbor-recent:${session.value.user?.id ?? "default"}`;
}
function loadRecentAccess() {
  try {
    recentAccess.value = JSON.parse(localStorage.getItem(recentKey()) || "[]");
  } catch {
    recentAccess.value = [];
  }
}
...
function openRecent(item: RecentAccessItem) {
  if (item.kind === "friend") {
    ...
    else void resolveSearchUser(item.id);
  } else if (item.kind === "world") {
    const world = visibleWorlds.value.find((value) => value.id === item.id);
    if (world) void openWorldDetail(world);
    else void resolveSearchWorld(item.id);
  }
}
...
async function loadFriendNetwork() {
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "共同好友网络读取失败";
  }
}

function networkScanCandidates(
  requestedIDs: string[] = [],
  limit = 100,
  includeFresh = false,
) {
  ...
  return [...(friends.value?.items ?? [])]
    ...
    .sort((left, right) => {
      if (hasRequested)
        return requestedIDs.indexOf(left.id) - requestedIDs.indexOf(right.id);
      const leftNode = nodeByID.get(left.id);
      const rightNode = nodeByID.get(right.id);
      if (leftNode?.scanned !== rightNode?.scanned)
        return leftNode?.scanned ? 1 : -1;
      if (left.online !== right.online) return left.online ? -1 : 1;
      return (leftNode?.scannedAt ?? "").localeCompare(
        rightNode?.scannedAt ?? "",
      );
    })
    .slice(0, limit);
...
async function startNetworkScan(requestedIDs: string[] = [], scanAll = false) {
  ...
  const candidates = networkScanCandidates(
    requestedIDs,
    scanAll ? Number.MAX_SAFE_INTEGER : 100,
    scanAll,
  );
  if (!candidates.length) {
    networkScanMessage.value = requestedIDs.length
      ? "没有找到可扫描的所选好友"
      : "没有需要更新的节点：已扫描数据均在 7 天有效期内";
    return;
  }
  ...
  try {
    ...
    if (networkScanCancelled)
      networkScanMessage.value = `已停止，本批完成 ${networkScanProcessed.value} 位`;
...
async function saveFriendAnnotation(value: {
  note: string;
  group: string;
  color: string;
  tags: string[];
  ...
  try {
    const saved = await api.updateFriendAnnotation(
    ...
    friendDetailError.value =
      cause instanceof Error ? cause.message : "本机备注保存失败";
  } finally {
...
function friendFromProfile(profile: UserProfile): Friend {
  return {
    ...
    online:
      ...
      (!!profile.location && profile.location !== "offline"),
  };
}
...
async function searchDiscoveryUsers(query: string) {
  ...
  error.value = "";
  try {
    const result = query.startsWith("usr_")
...
async function sendUserFriendRequest(profile: UserProfile) {
  ...
  try {
    const current =
      discoveryStatuses.value[profile.id] ??
      (await api.friendStatus(profile.id));
    discoveryStatuses.value = {
      ...discoveryStatuses.value,
      [profile.id]: current,
    };
    if (
      ...
      if (selectedFriend.value?.id === profile.id)
        selectedFriendStatus.value = current;
      return;
    }
    if (
      !window.confirm(
        `向“${profile.displayName}”发送好友请求？此操作会立即提交到 VRChat。`,
      )
    )
      return;
    ...
    if (selectedFriend.value?.id === profile.id)
      selectedFriendStatus.value = status;
  } catch (cause) {
...
async function sendFriendBoop(emojiId: string) {
  ...
  try {
    ...
    if (selectedFriend.value?.id === friend.id)
      boopMessage.value = "已发送到 VRChat";
  } catch (cause) {
    if (selectedFriend.value?.id === friend.id)
      friendDetailError.value =
        cause instanceof Error ? cause.message : "戳一戳发送失败";
  } finally {
    boopActingID.value = "";
  }
}
...
function selectSearchWorld(world: World) {
  const rest = (worlds.value?.items ?? []).filter(
...
async function openWorldDetail(world: World, location = "") {
  ...
  selectedInstanceLocation.value = location.startsWith(`${world.id}:`)
    ? location
    : "";
  ...
  try {
    ...
    if (selectedWorld.value)
      hydratedWorlds.value = {
        ...hydratedWorlds.value,
        [selectedWorld.value.id]: selectedWorld.value,
      };
  } catch {
    // Search results still provide a usable world summary while the detail endpoint is unavailable.
  }
  if (!selectedInstanceLocation.value) {
  ...
  try {
    ...
    selectedInstanceError.value =
      cause instanceof Error ? cause.message : "实例详情受权限或网络限制";
  } finally {
    worldDetailLoading.value = false;
  }
}
...
async function selectPublicInstance(location: string) {
  if (
    !selectedWorld.value ||
    !location.startsWith(`${selectedWorld.value.id}:`)
  )
    return;
  ...
  worldDetailLoading.value = true;
  try {
    selectedInstance.value = await api.instance(location);
  } catch (cause) {
    selectedInstanceError.value =
      cause instanceof Error ? cause.message : "实例详情受权限或网络限制";
  } finally {
...
async function saveWorldFavorite(note: string) {
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "本机世界收藏保存失败";
  } finally {
...
async function removeWorldFavorite() {
  if (
    !selectedWorld.value ||
    !window.confirm("只删除本机收藏和备注，不会修改 VRChat 收藏。继续吗？")
  )
    return;
  worldFavoriteSaving.value = true;
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "本机世界收藏删除失败";
  } finally {
    worldFavoriteSaving.value = false;
  }
}
...
async function addUpstreamFavorite(group: string) {
  if (
    !selectedWorld.value ||
    !window.confirm(
      `把“${selectedWorld.value.name}”加入 VRChat 上游收藏？此操作会立即写入你的 VRChat 账号。`,
    )
  )
    return;
  ...
  try {
    ...
    upstreamWorldFavorites.value = Object.fromEntries(
      items.map((item) => [item.world.id, item]),
    );
  } catch (cause) {
    error.value =
      cause instanceof Error ? cause.message : "VRChat 收藏保存失败";
  } finally {
    worldUpstreamSaving.value = false;
  }
}
...
async function removeUpstreamFavorite() {
  ...
  if (
    !favorite ||
    !window.confirm("从 VRChat 上游收藏中移除这个世界？本机备注不会删除。")
  )
    return;
  worldUpstreamSaving.value = true;
  try {
    await api.deleteUpstreamWorldFavorite(favorite.id);
    const next = { ...upstreamWorldFavorites.value };
    delete next[favorite.world.id];
    upstreamWorldFavorites.value = next;
  } catch (cause) {
    error.value =
      cause instanceof Error ? cause.message : "VRChat 收藏移除失败";
  } finally {
    worldUpstreamSaving.value = false;
  }
}
...
async function sendWorldInvite(receiverUserId: string) {
  const friend = friends.value?.items.find(
    (item) => item.id === receiverUserId,
  );
  if (!selectedWorld.value || !selectedInstanceLocation.value || !friend)
    return;
  if (
    !window.confirm(
      `邀请 ${friend.displayName} 前往“${selectedWorld.value.name}”？此操作会立即发送到 VRChat。`,
    )
  )
    return;
  inviteSending.value = true;
  try {
    await api.sendInvite(receiverUserId, selectedInstanceLocation.value);
  } catch (cause) {
    error.value = cause instanceof Error ? cause.message : "邀请发送失败";
  } finally {
...
async function runUpdateAction(action: "check" | "download" | "apply") {
  ...
  try {
    ...
    if (action === "apply" && window.confirm("安装更新会重启 VRC++。继续吗？"))
      await api.applyUpdate();
  } catch (cause) {
    error.value = cause instanceof Error ? cause.message : "更新操作失败";
  } finally {
...
async function loadActivity() {
  ...
  const [eventsResult, insightsResult] = await Promise.allSettled([
    api.activity(),
    api.activityInsights(),
  ]);
  if (eventsResult.status === "fulfilled")
    activityEvents.value = eventsResult.value;
  if (insightsResult.status === "fulfilled")
    activityInsights.value = insightsResult.value;
  if (
...
async function clearActivity() {
  if (
    !window.confirm(
      "将永久清除这台电脑上的活动事件历史。好友备注、关系网和登录会话不会删除。继续吗？",
    )
  )
    return;
  try {
...
async function loadNotifications() {
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "VRChat 通知读取失败";
  } finally {
...
async function actOnNotification(
  item: VrcNotification,
  action: "see" | "hide" | "accept",
) {
  const actionLabel =
    action === "accept"
      ? "接受这条好友请求"
...
async function refreshSystemCenter() {
  ...
  try {
    const [diag, cache, realtimeResult, logResult, updateResult] =
    ...
    if (updateResult.status === "fulfilled")
      updateStatus.value = updateResult.value;
    const failures = [
      diag,
      cache,
      realtimeResult,
...
async function clearMediaCache() {
  if (
    !window.confirm(
      "只清理可重新下载的图片缓存。登录会话、备注、关系网和布局不会删除。继续吗？",
    )
  )
    return;
  systemCenterLoading.value = true;
...
async function openCompassWorld(worldID: string, location = "") {
  ...
  if (known) {
    await openWorldDetail(known, location);
    return;
  }
  try {
    const result = await api.world(worldID);
    ...
    error.value = cause instanceof Error ? cause.message : "路线世界读取失败";
  }
}

async function hydrateVisibleWorlds(worldIDs: string[]) {
  const candidates = [...new Set(worldIDs)]
    .filter(
      (worldID) =>
        worldID.startsWith("wrld_") && !worldHydrationAttempted.has(worldID),
    )
    .slice(0, 12);
  for (const worldID of candidates) {
    ...
    try {
      ...
      const world = result.items[0];
      if (world)
        hydratedWorlds.value = { ...hydratedWorlds.value, [world.id]: world };
    } catch {
...
async function openSelfProfile() {
  ...
  try {
    ...
    if (!selfProfile.value)
      selfProfileError.value = l(
        "没有读取到个人资料",
        "Profile data was empty",
      );
  } catch (cause) {
    selfProfileError.value =
      cause instanceof Error
        ? cause.message
        : l("个人资料读取失败", "Unable to load profile");
  } finally {
...
async function saveSelfProfile(value: SelfProfileUpdate) {
  if (
    !window.confirm(
      l(
        "这些资料会立即写入你的 VRChat 账号，继续吗？",
        "These changes will be written to your VRChat account now. Continue?",
      ),
    )
  )
    return;
  ...
  try {
    ...
    selfProfileError.value =
      cause instanceof Error
        ? cause.message
        : l("个人资料保存失败", "Unable to save profile");
  } finally {
...
async function openFriend(friend: Friend) {
  recordRecent({
    ...
    imageUrl: preferredFriendAvatar(friend),
  });
  ...
  const knownFriend =
    friends.value?.items.some((item) => item.id === friend.id) ?? false;
  if (knownFriend)
    selectedFriendStatus.value = {
      isFriend: true,
  ...
  const profileRequest = api
    ...
    .then((result) => {
      ...
      if (requestID === friendDetailRequest)
        friendProfile.value = loadedProfile;
    })
    .catch((cause) => {
      if (requestID === friendDetailRequest)
        friendDetailError.value =
          cause instanceof Error ? cause.message : "好友资料读取失败";
    });
  const mutualRequest = api
    .mutualFriends(friend.id)
    .then((result) => {
      if (requestID === friendDetailRequest) mutualFriends.value = result.items;
    })
    .catch(() => {
      /* profile remains usable when mutuals are unavailable */
    });
  const insightRequest = api
    .friendActivityInsights(friend.id)
    .then((result) => {
      if (requestID === friendDetailRequest)
        friendActivityInsights.value = result;
    })
    .catch(() => {
      /* empty local insight state is explicit in the drawer */
    });
  const statusRequest = (
    knownFriend
      ? Promise.resolve(selectedFriendStatus.value!)
      : api.friendStatus(friend.id)
  )
    .then((result) => {
      if (requestID === friendDetailRequest)
        selectedFriendStatus.value = result;
    })
    .catch(() => {
      /* unknown relationship state disables no existing read-only feature */
    });
  await Promise.all([
    profileRequest,
    mutualRequest,
  ...
  if (worldID) {
    try {
      const result = await api.world(worldID);
      if (requestID === friendDetailRequest)
        friendWorld.value = result.items[0] ?? null;
    } catch {
...
function openMutualFriend(mutual: MutualFriend) {
  const existing = friends.value?.items.find(
    (friend) => friend.id === mutual.id,
  );
  void openFriend(
    existing ?? {
      id: mutual.id,
...
async function loadAppData() {
  ...
  const tasks = [
    ...
    api.realtimeStatus().then((value) => {
      realtime.value = value;
    }),
    api.friendAnnotations().then((value) => {
      annotations.value = Object.fromEntries(
        value.map((item) => [item.userId, item]),
      );
    }),
    api.worldFavorites().then((value) => {
      worldFavorites.value = Object.fromEntries(
        value.map((item) => [item.world.id, item]),
      );
    }),
    api.upstreamWorldFavorites().then((value) => {
      upstreamWorldFavorites.value = Object.fromEntries(
        value.map((item) => [item.world.id, item]),
      );
    }),
    api.favoriteGroups().then((value) => {
      favoriteGroups.value = value;
    }),
    api.gameLogStatus().then((value) => {
      gameLogStatus.value = value;
    }),
    api.updateStatus().then((value) => {
      updateStatus.value = value;
    }),
    api.activity(30, 500).then((value) => {
      activityEvents.value = value;
    }),
    api.activityInsights(30).then((value) => {
      activityInsights.value = value;
    }),
    api.notifications().then((value) => {
      notifications.value = value;
    }),
    api.friendNetwork().then((value) => {
      friendNetwork.value = value;
    }),
  ];
...
async function verifyTwoFactor() {
  ...
  error.value = "";
  try {
    session.value = await api.verifyTwoFactor(
      twoFactorType.value,
...
watch(uiScale, (value) => {
  uiScale.value = applyUIScale(value);
});
watch(locale, (value) => {
  locale.value = applyInterfaceLocale(value);
});
onBeforeUnmount(() => {
  networkScanCancelled = true;
  closeEvents();
...
<template>
  <div
    class="app-shell"
    :class="{ 'console-mode': session.status === 'authenticated' }"
  >
    <aside
      v-if="
    ...
    <template v-if="session.status !== 'authenticated'">
      <header class="topbar">
        <a class="brand" href="#"
          ><span class="brand-mark"
            ><img :src="'/assets/vrc-plus-plus-mark.png'" alt="" /></span
          ><span
            ><strong>VRC++</strong><small>本地 VRChat 管理工具</small></span
          ></a
        >
        <div class="topbar-actions">
          <button
            ...
            @click="toggleTheme"
          >
            <Sun v-if="theme === 'dark'" :size="17" /><Moon
              v-else
              :size="17"
            /></button
          ><button class="top-action" @click="settingsOpen = true">
            <Settings2 :size="16" /> 网络设置
          </button>
          <div class="topbar-status">
            <span class="pulse-dot"></span><span>本机网关</span
      ...
      <main class="landing-main">
        <section class="hero modern-hero">
          <div class="eyebrow">
            <ShieldCheck :size="15" /> 账号会话只保存在这台电脑
          </div>
          <h1>VRChat 的本地控制台</h1>
          <p>
            查看好友状态、搜索世界并诊断网络连接。页面资源全部内置，API、实时消息和图片可以统一使用你的本机代理。
          </p>
          <div class="hero-actions">
            ...
            >
              <LoaderCircle v-if="probing" class="spin" :size="18" /><RefreshCw
                v-else
                :size="18"
              />{{ probing ? "正在检测" : "检测当前线路" }}</button
            ><button class="ghost-button" @click="settingsOpen = true">
              <PlugZap :size="17" /> {{ network.label }}
            </button>
          </div>
          <div class="hero-proof">
            <span><CheckCircle2 :size="15" /> 无公共 CDN</span
            ><span><CheckCircle2 :size="15" /> 图片本机缓存</span
            ><span><CheckCircle2 :size="15" /> Cookie 系统加密</span>
          </div>
        </section>
        ...
        <section class="landing-grid" :class="{ loading }">
          <article class="panel login-panel">
            <div class="panel-heading">
              ...
              <div class="icon-tile"><KeyRound :size="22" /></div>
            </div>
            <form
              v-if="session.status === 'two_factor_required'"
              class="login-form"
              @submit.prevent="verifyTwoFactor"
            >
              <label
                >验证方式<select v-model="twoFactorType">
                  <option
                    v-for="method in session.methods"
                    :key="method"
                    :value="method"
                  >
                    {{
              ...
              ><label
                >验证码<input
                  ...
                  placeholder="输入验证码"
                  required /></label
              ><button class="primary-button full" :disabled="submitting">
                完成验证</button
              ><small class="form-note"
                >验证码仅由本机网关发送到 VRChat。</small
              >
            </form>
            <form v-else class="login-form" @submit.prevent="login">
              ...
              ><button class="primary-button full" :disabled="submitting">
                ...
                /><LockKeyhole v-else :size="18" />{{
                  submitting ? "正在安全连接" : "连接我的 VRChat"
                }}</button
              ><small class="form-note"
                >密码不落库、不进入浏览器存储，仅保存系统加密后的登录
                Cookie。</small
              >
            </form>
          </article>
          <article class="panel route-panel">
            <div class="panel-heading">
              <div>
                <span class="panel-kicker">连接状态</span>
                <h2>{{ overallLabel }}</h2>
              </div>
              <div
                class="status-orb"
                :data-state="diagnostics?.overall ?? 'unknown'"
              >
                <Activity :size="22" />
              </div>
            </div>
            <div class="network-summary">
              ...
              <button @click="settingsOpen = true">切换</button>
            </div>
            <div class="check-list">
              ...
              <div v-if="!diagnostics?.checks.length" class="empty-state">
                等待本机线路检测
              </div>
            </div>
          </article>
    ...
    <template v-else>
      <aside class="sidebar">
        <div class="sidebar-brand">
          <span class="brand-mark"
            ><img :src="'/assets/vrc-plus-plus-mark.png'" alt=""
          /></span>
          <div>
            <strong>VRC++</strong><small>{{ bootstrap?.version }}</small>
          </div>
        </div>
        <nav>
          <button
          ...
          >
            <Gauge :size="19" /> {{ l("总览", "Overview") }}</button
          ><button
            :class="{ active: activeView === 'compass' }"
            @click="selectView('compass')"
          >
            <Sparkles :size="19" /> {{ l("罗盘", "Compass") }}</button
          ><button
            :class="{ active: activeView === 'journey' }"
            @click="selectView('journey')"
          >
            <Route :size="19" /> {{ l("旅程", "Journey") }}</button
          ><button
            :class="{ active: activeView === 'friends' }"
            @click="selectView('friends')"
          >
            <Users :size="19" /> {{ l("好友", "Friends")
            }}<span>{{ onlineFriends }}</span></button
          ><button
            :class="{ active: activeView === 'focus' }"
            @click="selectView('focus')"
          >
            <Heart :size="19" /> {{ l("特别关心", "Watchlist") }}</button
          ><button
            :class="{ active: activeView === 'alerts' }"
            @click="selectView('alerts')"
          >
          ...
          >
            <Network :size="19" /> {{ l("关系网", "Network") }}</button
          ><button
            :class="{ active: activeView === 'worlds' }"
            @click="selectView('worlds')"
          >
            <Globe2 :size="19" /> {{ l("世界", "Worlds") }}</button
          ><button
            :class="{ active: activeView === 'groups' }"
            @click="selectView('groups')"
          >
            <Building2 :size="19" /> {{ l("群组", "Groups") }}</button
          ><button
            :class="{ active: activeView === 'avatars' }"
            @click="selectView('avatars')"
          >
            <Box :size="19" /> {{ l("头像", "Avatars") }}</button
          ><button
            :class="{ active: activeView === 'activity' }"
            @click="selectView('activity')"
          >
            <History :size="19" /> {{ l("历史", "History") }}</button
          ><button
            :class="{ active: activeView === 'notifications' }"
            @click="selectView('notifications')"
          >
            <Bell :size="19" /> {{ l("通知", "Notifications")
        ...
        <div class="sidebar-bottom">
          ...
          ><button @click="toggleLocale">
            <Languages :size="18" />
            {{ locale === "zh-CN" ? "English" : "简体中文" }}</button
          ><button @click="displaySettingsOpen = true">
            <ALargeSmall :size="18" /> {{ l("字体大小", "Text size") }}
            <span>{{ Math.round(uiScale * 100) }}%</span></button
          ><button @click="settingsOpen = true">
            <Settings2 :size="18" /> {{ l("网络与缓存", "Network & cache") }}
          </button>
          <div class="sidebar-user">
            <button
              class="sidebar-user-main"
              :title="l('查看和编辑我的资料', 'View and edit my profile')"
              @click="openSelfProfile"
            >
              <img v-if="userImage()" :src="userImage()" alt="" /><span
                v-else
                class="mini-avatar"
                >{{ session.user?.displayName?.slice(0, 1) }}</span
              ><span
                ><strong>{{ session.user?.displayName }}</strong
                ><small>{{ realtimeLabel }}</small></span
              ></button
            ><button :title="l('退出', 'Sign out')" @click="logout">
              <LogOut :size="17" />
            </button>
          </div>
        </div>
      ...
      <main class="console-main">
        <header class="console-header">
          <div>
            <span class="panel-kicker">{{ viewCopy[0] }}</span>
            <h1>{{ viewCopy[1] }}</h1>
            <p>{{ viewCopy[2] }}</p>
          </div>
          <div class="console-actions">
            <GlobalSearch
              ...
              @select-friend="openFriend"
              @select-world="selectSearchWorld"
              @resolve-user="resolveSearchUser"
              @resolve-world="resolveSearchWorld"
            /><button
              class="theme-button"
              :title="theme === 'dark' ? '切换浅色模式' : '切换深色模式'"
              @click="toggleTheme"
            ...
            ><span class="route-pill"
              ><PlugZap :size="14" /> {{ network.label }}</span
            ><button
              class="icon-button"
              title="刷新"
              @click="refreshDiagnostics"
        ...
        <section class="console-grid" :class="{ loading: dataLoading }">
          ...
          <JourneyView
            ...
            @open-world="openCompassWorld"
            @load-network="loadFriendNetwork"
            @refresh-system="refreshSystemCenter"
            @hydrate-worlds="hydrateVisibleWorlds"
          />
          <RecentAccess
            v-if="activeView === 'overview'"
            :items="recentAccess"
          ...
          <FriendNetworkView
            ...
            :scan-processed="networkScanProcessed"
            :scan-total="networkScanTotal"
            :scan-estimate="networkScanEstimate"
            :scan-message="networkScanMessage"
            :media-url="api.mediaUrl.bind(api)"
            :layout-key="session.user?.id ?? 'default'"
            :annotations="annotations"
            @start-scan="startNetworkScan"
            @scan-all="startNetworkScan([], true)"
            @scan-friends="startNetworkScan"
            @stop-scan="stopNetworkScan"
            @open-friend="openNetworkFriend"
          />
          <FriendListView
          ...
          <ConcernedFriendView
            ...
            @open-friend="openNetworkFriend"
            @open-world="openCompassWorld"
            @network-updated="loadFriendNetwork"
          />
          <DiscoveryView
          ...
          <ActivityView
            ...
            @clear="clearActivity"
            @select-user="selectActivityUser"
          />
          <NotificationCenter
            ...
            :acting-id="notificationActingID"
          ...
          />
          <AvatarCabinet
            v-if="activeView === 'avatars'"
            :storage-key="session.user?.id ?? 'default'"
            :media-url="api.mediaUrl.bind(api)"
          />
          <WorldMemory
            v-if="activeView === 'worlds'"
            :events="activityEvents"
            :worlds="visibleWorlds"
            :friends="friends?.items ?? []"
          ...
          <article
            ...
            :class="{ wide: activeView === 'friends' }"
          >
            <div class="panel-heading compact">
              <div>
                <span class="panel-kicker">好友</span>
                <h2>好友状态</h2>
              </div>
              <div class="data-heading-actions">
                <span
                  class="source-chip"
                  :data-source="friends?.source ?? 'live'"
                  >{{
                    friends?.source === "cache" ? "本地快照" : "实时数据"
                  }}</span
                ><span class="realtime-chip" :data-state="realtime.state"
                  ><Radio :size="13" /> {{ realtimeLabel }}</span
                >
              </div>
            </div>
            <p v-if="friends?.message" class="snapshot-note">
              {{ friends.message }}
            </p>
            <div class="friend-grid">
              <button
                ...
                :style="
                  annotations[friend.id]?.color
                    ? {
                        borderLeftColor: annotations[friend.id].color,
                        borderLeftWidth: '3px',
                      }
                    : undefined
                "
              ...
              >
                <img
                  v-if="friendImage(friend)"
                  :src="friendImage(friend)"
                  alt=""
                  :loading="activeView === 'overview' ? 'eager' : 'lazy'"
                  decoding="async"
                />
                <span v-else class="friend-fallback">{{
                ...
                <span class="friend-copy"
                  ...
                  ><small
                    >{{
                      annotations[friend.id]?.group || friendLocation(friend)
                    }}
                    ·
                    {{
                      annotations[friend.id]?.tags.join(" · ") ||
                      friend.platform ||
                      friend.lastPlatform ||
                      "未知平台"
                    }}</small
                  ><span
                    v-if="
                      annotations[friend.id]?.note || friend.statusDescription
                    "
              ...
              <div v-if="!friends?.items.length" class="empty-state">
                <Users :size="19" /> 暂无好友快照
              </div>
            </div>
            <button
              v-if="activeView === 'overview'"
          ...
          <article
            v-if="showWorlds"
            class="panel worlds-panel"
            :class="{ wide: activeView === 'worlds' }"
          >
            <div class="panel-heading compact world-heading">
              <div>
                <span class="panel-kicker">世界</span>
                <h2>世界与收藏</h2>
              </div>
              <span class="source-chip" :data-source="worlds?.source ?? 'live'"
                >{{ Object.keys(upstreamWorldFavorites).length }} 个 VRChat
                收藏</span
              >
            </div>
            <form class="world-search" @submit.prevent="searchWorlds">
              <Search :size="17" /><input
                v-model="worldSearch"
                placeholder="搜索世界名称"
                aria-label="世界名称"
              /><button type="submit" :disabled="dataLoading">搜索</button>
            </form>
            <div class="world-grid">
              <button
                ...
                type="button"
                class="world-card"
                @click="openWorldDetail(world)"
              >
                <div class="world-image">
                  <img
                    v-if="worldImage(world)"
                    :src="worldImage(world)"
                    alt=""
                    :loading="activeView === 'overview' ? 'eager' : 'lazy'"
                    decoding="async"
                  /><Globe2 v-else :size="28" /><span
                    v-if="world.occupants !== undefined"
                    >{{ world.occupants }} 在线</span
                  >
                </div>
                <div class="world-copy">
                  <strong>{{ world.name }}</strong
                  ><small
                    >{{ world.authorName || "未知作者" }} · 容量
                    {{
                      world.recommendedCapacity || world.capacity || "—"
                    }}</small
                  >
                  <div class="world-badges">
                    <span v-if="upstreamWorldFavorites[world.id]"
                      >VRChat 收藏</span
                    ><span v-if="worldFavorites[world.id]">本机收藏</span>
                  </div>
                  <p>
                    {{
                      worldFavorites[world.id]?.note ||
                      world.description ||
                      "暂无世界简介"
                    }}
                  </p>
              ...
              <div v-if="!visibleWorlds.length" class="empty-state">
                <Globe2 :size="19" /> 没有找到匹配世界
              </div>
            </div>
            <button
              v-if="activeView === 'overview'"
    ...
    <SelfProfileDialog
      v-if="selfProfileOpen"
      :profile="selfProfile"
      :loading="selfProfileLoading"
      :saving="selfProfileSaving"
      :error="selfProfileError"
      :locale="locale"
      :media-url="api.mediaUrl.bind(api)"
      @close="selfProfileOpen = false"
      @save="saveSelfProfile"
    />
    ...
    >
      <section class="settings-modal">
        <div class="modal-heading">
          <div>
            <span class="panel-kicker">本机设置</span>
            <h2>网络、日志与更新</h2>
          </div>
        ...
        <SystemCenter
          ...
          :network="network"
          :cache="cacheStats"
          :loading="systemCenterLoading"
          @refresh="refreshSystemCenter"
          @clear-media="clearMediaCache"
        />
        <div class="runtime-strip">
          <span
            ><Gamepad2 :size="16" /><b>游戏日志</b
            >{{
              gameLogStatus.state === "watching"
                ? "监视中"
                : gameLogStatus.state === "waiting"
                  ? "等待 VRChat"
                  : gameLogStatus.state
            }}
            · {{ gameLogStatus.events }} 条</span
          ><span
        ...
        <div class="mode-grid">
          <button
            v-for="mode in [
              ['system', '跟随系统', '读取 HTTP_PROXY / HTTPS_PROXY'],
              ['direct', '强制直连', '忽略所有代理环境变量'],
              ['http', 'HTTP 代理', '适合 Clash HTTP 端口'],
              ['socks5', 'SOCKS5', '适合本机 SOCKS5 端口'],
            ] as const"
            :key="mode[0]"
            :class="{ active: networkMode === mode[0] }"
            @click="networkMode = mode[0]"
          >
            <span>{{ mode[1] }}</span
            ><small>{{ mode[2] }}</small>
          </button>
        ...
        <label
          v-if="networkMode === 'http' || networkMode === 'socks5'"
          class="proxy-field"
          >代理地址<input
            v-model="proxyUrl"
            :placeholder="
            ...
            "
          /><small
            >仅支持本机或自有代理；URL 中不保存用户名和密码。</small
          ></label
        >
        <div class="modal-note">
          <GlobeLock :size="18" /><span
            >应用不依赖公共前端
        ...
        <button
          ...
          @click="saveNetwork"
        >
          <LoaderCircle v-if="savingNetwork" class="spin" :size="18" /><PlugZap
            v-else
            :size="18"
          />应用网络配置并重新检测
        </button>
    ...
    <div
      v-if="displaySettingsOpen"
      class="modal-backdrop"
      @click.self="displaySettingsOpen = false"
    >
      <section class="settings-modal display-settings-modal">
        <div class="modal-heading">
          <div>
            <span class="panel-kicker">{{ l("显示设置", "Display") }}</span>
            <h2>{{ l("字体与语言", "Text and language") }}</h2>
          </div>
          <button class="icon-button" @click="displaySettingsOpen = false">
            <X :size="18" />
          </button>
        </div>
        <div class="font-scale-preview">
          ...
          <div>
            <strong
              >{{ l("字体大小", "Text size") }}
              {{ Math.round(uiScale * 100) }}%</strong
            ><span>{{
              l(
                "跟随 Windows DPI，仅调整文字，避免界面二次放大。",
                "Uses Windows DPI and adjusts text only.",
              )
            }}</span>
          </div>
        </div>
        ...
        <div class="language-setting">
          <Languages :size="19" />
          <div>
            <strong>{{ l("界面语言", "Language") }}</strong
            ><span>{{ locale === "zh-CN" ? "简体中文" : "English" }}</span>
          </div>
          <button @click="toggleLocale">
            {{ locale === "zh-CN" ? "Switch to English" : "切换到简体中文" }}
          </button>
        </div>
      </section>
    </div>
