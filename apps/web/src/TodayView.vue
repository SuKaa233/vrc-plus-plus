...
<template>
  <section class="today-view wide-view">
    ...
    <div class="today-columns">
      <article class="today-panel daily-brief"><header><div><ListChecks :size="17" /><strong>今日简报</strong></div><span>最多 5 条 · 本机生成</span></header><button v-for="item in brief" :key="item.id" class="brief-row" @click="openBrief(item)"><span :data-kind="item.kind"></span><div><strong>{{ item.title }}</strong><small>{{ item.summary }}</small></div><ChevronRight :size="14" /></button><p v-if="!brief.length" class="today-empty">当前没有需要优先处理的场景；继续积累本机记录后会自动生成。</p></article>
