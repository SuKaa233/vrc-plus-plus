...
import { buildCoverageMap, buildFriendReplay, buildIntersectionChanges, buildMovementChains } from './product-insights'
...
const coverage = computed(() => buildCoverageMap(props.friends, props.events, props.network))
const replay = computed(() => focusedUserId.value ? buildFriendReplay(props.events, focusedUserId.value, props.worlds, replayDays.value) : [])
...
<template>
  <section class="motion-view">
    ...
    <div class="motion-layout">
      ...
      <article v-if="focusedUserId" class="motion-panel replay-panel panel"><header><div><span class="panel-kicker">{{ l('回放','Replay') }}</span><h3>{{ friendName(focusedUserId) }} · {{ replayDays }} {{ l('天动态','day activity') }}</h3></div><span>{{ replay.length }} {{ l('条活动记录','activity records') }}</span></header><div class="replay-list"><button v-for="item in replay" :key="item.id" @click="item.worldId&&emit('openWorld',item.worldId,'')"><i :data-source="item.source"></i><div><strong>{{ item.title }}</strong><small>{{ item.detail }} · {{ item.source==='gameLog'?l('游戏记录','game record'):l('实时动态','live activity') }}</small></div><time>{{ formatTime(item.observedAt) }}</time></button><p v-if="!replay.length">{{ l('这个时间窗口内没有活动记录。','No activity records in this window.') }}</p></div></article>
      ...
      <article class="motion-panel coverage-panel panel">
        ...
        <div class="coverage-list"><button v-for="item in coverage" :key="item.userId" @click="emit('openFriend',item.userId)"><span :data-level="item.level"></span><div><strong>{{ friendName(item.userId) }}</strong><small>{{ item.observationDays }} {{ l('个记录日','recorded days') }} · {{ item.eventCount }} {{ l('条动态','events') }} · {{ l('实时','live') }} {{ item.pipelineEvents }} / {{ l('游戏','game') }} {{ item.gameLogEvents }}</small><em>{{ item.scanned?l('共同关系已读取','connections loaded'):l('共同关系尚未读取','connections not loaded') }} · {{ item.visibleLocationEvents }} {{ l('条含世界位置','events with world') }}</em></div><b>{{ item.score }} · {{ item.level }}</b></button><p v-if="!coverage.length">{{ l('尚无好友数据。','No friend coverage data.') }}</p></div>
      </article>
    </div>
  </section>
</template>
...
.replay-panel{grid-column:1/-1}.replay-list,.evolution-columns>section,.coverage-list{max-height:420px;overflow:auto}.replay-list button{width:100%;display:grid;grid-template-columns:8px 1fr auto;align-items:center;gap:9px;padding:9px 4px;border:0;border-top:1px solid var(--line);background:transparent;color:var(--ink);text-align:left;content-visibility:auto;contain-intrinsic-size:44px}.replay-list i{width:7px;height:7px;border-radius:50%;background:var(--accent)}.replay-list i[data-source=gameLog]{background:var(--success)}.replay-list strong,.replay-list small{display:block}.replay-list strong{font-size:10px}.replay-list small,.replay-list time{margin-top:2px;color:var(--muted);font-size:8px}.coverage-list button{content-visibility:auto;contain-intrinsic-size:48px}.coverage-list em{display:block;margin-top:2px;color:var(--muted);font-size:8px;font-style:normal}.coverage-list button>span[data-level='充分']{background:var(--success)}
</style>
