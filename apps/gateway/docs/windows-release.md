...
安装向导默认安装到 `%LOCALAPPDATA%\Programs\VRC++`，不要求管理员权限。手动安装始终显示目录页；覆盖安装默认沿用旧目录，但允许用户修改。开始菜单和桌面快捷方式始终创建；开机启动默认关闭。卸载只删除程序文件和快捷方式，不删除 `%APPDATA%\VRC++` 中的账号会话、历史和本机设置。
...
程序启动 12 秒后自动检查一次，之后每 6 小时检查一次；页面每 30 秒读取本机更新状态。发现新版本时显示更新提示，用户确认后下载安装程序，再以静默模式覆盖原安装目录并重启。静默更新不会显示目录页，但仍会确保桌面和开始菜单快捷方式存在。更新请求会跟随应用中的直连、系统代理或本机代理设置。升级只替换程序文件，不删除 `%APPDATA%\VRC++` 数据。
